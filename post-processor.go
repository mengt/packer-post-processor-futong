...
func (p *PostProcessor) GetDiskFileInfo(artifact packer.Artifact) (string, int, error) {
	...
	log.Printf("Futong format: %s", abqFormat)
...
func (p *PostProcessor) Configure(raws ...interface{}) error {
	...
	if p.config.ApiUrl == "" {
		errs = packer.MultiErrorAppend(errs, fmt.Errorf("Futong API URL is missing!"))
	}

	if p.config.ApiUsername == "" && p.config.AppKey == "" {
		errs = packer.MultiErrorAppend(errs, fmt.Errorf("Futong API username or Oauth app keys are missing!"))
	}

	if p.config.ApiUsername != "" && p.config.ApiPassword == "" {
		errs = packer.MultiErrorAppend(errs, fmt.Errorf("Futong API username provided but no password!"))
	}
